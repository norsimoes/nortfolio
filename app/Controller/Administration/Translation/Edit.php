 ...
 * Edit
 *
 * Edit an existing translation record.
 */
class Edit extends App
{
    ...
    public function index(int $translationId = 0): void
    {
        ...
        // Get data
        $data['translation-data'] = $translationModel->getTranslationData($translationId);
     ...
     * Register
     *
     * Handles the registration of an existing translation.
     *
     * @throws \Exception
     */
    public function register(int $translationId = 0): void
    {
        ...
        $this->checkPost($this->_moduleData['edit']->route, [$translationId]);
         ...
         */
        if ($translationModel->isDuplicated($callSign, 'call_sign', $translationId)) {

            // Set message
            $this->session->setMessage('info', str_replace('{txt}', $callSign, $this->_i18n['MsgInformation'][1]));

            // Redirect
            $this->goToRoute($this->_moduleData['edit']->route, [$translationId]);
        }

        // Update translation record
        $operationStatus = $translationModel->edit($translationId, $callSign, $valueArr);
