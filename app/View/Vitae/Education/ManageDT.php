...
if (isset($data['records'])) {
    ...
    foreach ($data['records'] as $row) {
        ...
        if (\Lib\Access::getInstance()->module($data['module-id-edit'])) {
            ...
            $link->setAttr('href', $data['url-edit'] . $row->education_id . '/');
        ...
        if (\Lib\Access::getInstance()->module($data['module-id-delete'])) {
            ...
            $link->setAttr('data-url', $data['url-delete'] . $row->education_id . '/');
            $link->setAttr('data-icon', 'fas fa-trash');
            $link->setAttr('data-color', 'danger');
            $link->setAttr('data-title', str_replace('{txt}', $row->course, $i18nCore['Manage'][6]));
        ...
        $rowArr[0] = $row->education_id;
        $rowArr[1] = $row->course;
        $rowArr[2] = $row->institution;
        $rowArr[3] = $row->start;
        $rowArr[4] = $row->end;
