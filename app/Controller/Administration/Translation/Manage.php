 ...
 * Manage
 *
 * Manage existing translation records.
 */
class Manage extends App
{
    ...
    public function index(): void
    {
        ...
        $this->template->loadView('Administration/Translation/Manage', $data);
    ...
    public function dataTables(): void
    {
        ...
        if ($this->input->get('__source')) {
            ...
            $data['url-get-group'] = $this->_moduleData['group']->url ?? '';
            $data['url-clone-form'] = $this->_moduleData['copy']->url . 'cloneTranslationForm/';
            $data['url-clone'] = $this->_moduleData['copy']->url . 'cloneTranslation/';
            ...
            $data['module-id-clone'] = $this->_moduleData['clone']->id ?? 0;

            /*
             * Data Tables total records
             */
            $totalRecords = $translationModel->getAll(0, 0, '', $multiSort);
            $data['DT_recordsTotal'] = count($totalRecords);

            /*
             * Data Tables total records after filters applied
             */
            $recordsAfterFilter = $translationModel->getAll(0, 0, $search, $multiSort, $mySqlFilter);
            $data['DT_recordsFiltered'] = count($recordsAfterFilter);

            /*
             * Data Tables records after filters and pagination
             */
            $records = $translationModel->getAll($start, $length, $search, $multiSort, $mySqlFilter);
             ...
             * Render the view
             */
            $this->loader->view('Administration/Translation/ManageDT', $data);
            exit();
        }
    }
