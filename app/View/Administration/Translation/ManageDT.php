...
if (isset($data['records'])) {
    ...
    foreach ($data['records'] as $row) {
        ...
        $folderColor = ($row->group_count > 0) ? 'primary' : 'secondary';

        $link->setAttr('href', $data['url-get-group'] . $row->translation_id . '/');
        $link->setAttr('title', $i18n['Manage'][2] . ' ' . $row->group_count);
        ...
        if (\Lib\Access::getInstance()->module($data['module-id-edit'])) {
            ...
            $link->setAttr('href', $data['url-edit'] . $row->translation_id . '/');
        ...
        if (\Lib\Access::getInstance()->module($data['module-id-clone'])) {
            ...
            $link->setAttr('title', $i18n['ModalCloneTranslation'][1]);
            $link->setAttr('class', 'btn dt-btn text-secondary');
            $link->setAttr('data-bs-toggle', 'modal');
            $link->setAttr('data-bs-target', '#j-template-modal');
            $link->setAttr('data-width', '500px');
            $link->setAttr('data-modal-title', $i18n['ModalCloneTranslation'][1]);
            $link->setAttr('data-get-url', $data['url-clone-form']);
            $link->setAttr('data-post-url', $data['url-clone'] . $row->translation_id . '/');
        ...
        if (\Lib\Access::getInstance()->module($data['module-id-delete'])) {
            ...
            $link->setAttr('data-url', $data['url-delete'] . $row->translation_id . '/');
            $link->setAttr('data-icon', 'fas fa-trash');
            $link->setAttr('data-color', 'danger');
            $link->setAttr('data-title', str_replace('{txt}', $row->value, $i18n['Manage'][5]));
            $link->setAttr('data-text', $i18n['Manage'][6] . '<br /><b>' . $i18nCore['Manage'][7] . '</b>');
        ...
        $rowArr[0] = $row->translation_id;
        $rowArr[1] = $row->call_sign;
        $rowArr[2] = $row->value;
        $rowArr[3] = $row->group_count;
        $rowArr[4] = $cellAction;
