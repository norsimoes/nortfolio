 ...
 * ----------------------------------------------------------------------------
 */
function modulesHtml($moduleData = [], $controllerActions = [], $defaultAction = '', $userPermissions = [], $rolePermissions = [], $level = 1): string
{
    $modulesHtml = '';

    foreach ($moduleData as $moduleId => $module) {

        $checked = in_array($moduleId, $userPermissions) ? 'checked' : '';
        ...
        if (in_array($moduleId, $rolePermissions)) {
            ...
            <td class="text-center">
                <span class="fa fa-fw fa-minus text-muted small"></span>
            </td>  
            ';
        }
        ...
        if (!empty($controllerActions)) {
            ...
            foreach ($controllerActions as $actionName => $controllerAction) {
                ...
                if (in_array($controllerAction, $actions)) {
                    ...
                    $checked = in_array($actionModuleId, $userPermissions) ? 'checked' : '';

                    if (in_array($actionModuleId, $rolePermissions)) {
        ...
        if (is_array($module['child'])) $modulesHtml .= modulesHtml($module['child'], $controllerActions, $defaultAction, $userPermissions, $rolePermissions, $level + 1);
    }
...
if (!empty($data['interfaces'])) {
    ...
    foreach ($data['interfaces'] as $interface) {
        ...
        $modules = modulesHtml($data['modules'][$interface->module_id], $data['controller-actions'], $data['default-action'], $data['user-permissions'], $data['role-permissions']);
        ...
        $checked = (in_array($interface->module_id, $data['user-permissions'])) ? 'checked' : '';
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col">
                                ...
                                <div class="card-title"><?= $data['user-data']->name ?></div>
                                <div class="card-desc"><?= $data['user-data']->role_name ?></div>
