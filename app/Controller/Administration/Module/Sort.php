...
class Sort extends App
{
    ...
    public function index(int $parentModuleId = 0): void
    {
        ...
        // Set module data
        $data['module-data'] = $moduleModel->getAllByParentId(0, 0, '', 'position ASC', [], $parentModuleId);
        ...
        $this->template->loadView('Administration/Module/Sort', $data);
    ...
    public function sort(): string
    {
        ...
        if (is_array($positionArr) && count($positionArr) > 0) {
            ...
            foreach ($positionArr as $position => $id) {
                ...
                if ($moduleModel->updatePosition($id, $position + 1)) {
                    $successCount ++;
                } else {
                    $errorCount ++;
                }
