...
if (isset($data['records'])) {
    ...
    foreach ($data['records'] as $row) {
        ...
        if (\Lib\Access::getInstance()->module($data['module-id-edit'])) {
            ...
            $link->setAttr('href', $data['url-edit'] . $row->blank_id . '/');
        ...
        if (\Lib\Access::getInstance()->module($data['module-id-delete'])) {
            ...
            $link->setAttr('data-url', $data['url-delete'] . $row->blank_id . '/');
        ...
        $rowArr[0] = $row->blank_id;
        $rowArr[1] = $row->call_sign;
        $rowArr[2] = $row->name;
        $rowArr[3] = $row->description;
        $rowArr[4] = $row->position;
        $rowArr[5] = $cellAction;
